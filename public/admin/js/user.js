			...
			function getUsers(searchObj) {
				...
				updateUsersTableHeaderSort();
				  ...
				  $.ajax({
					...
					<td>${ result[i] ? result[i].id ? result[i].id : ' '  : ' '  }</td>
						...
						<td>${ result[i] ? result[i].email ? result[i].email : ' '  : ' '   }</td><td>${ [  ][result[i].role_id]}</td><td><span class="btn btn-link btn-sm" onclick="editUserModal(  '${result[i].first_name}', '${result[i].last_name}', '${result[i].phone}', '${result[i].email}', '${result[i].password}', '${result[i].role_id}', ${result[i].id} )">Edit</span><span class="btn btn-link btn-sm" onclick="deleteUserModal(${result[i].id})">Delete</span></td>
				          </tr>
				        `);
				      }
				...
				function updateUser()  {
				  $.ajax({
				    ...
				    method: "POST",
				    data: {
				    	first_name : $("#editUserFirstNameInput").val(),last_name : $("#editUserLastNameInput").val(),phone : $("#editUserPhoneInput").val(),email : $("#editUserEmailInput").val(),password : $("#editUserPasswordInput").val(),role_id : $("#editUserRoleIdInput").val(),id : $("#editUserId").val(),				    
				      token: Cookies.get("token"),
				    },
				    success: function (result) {
				      console.log(result);
				...
				function editUserModal(first_name,last_name,phone,email,password,role_id,id) {
				  ...
				  $("#editUserId").val(id);
				  $("#editFirstNameInput").val(first_name);$("#editLastNameInput").val(last_name);$("#editPhoneInput").val(phone);$("#editEmailInput").val(email);$("#editPasswordInput").val(password);$("#editRoleIdInput").val(role_id);
				}

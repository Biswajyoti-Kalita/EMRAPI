  ...
  initializeApi: function (app) {
    ...
    app.post(
      ...
      async function (req, res) {
        ...
        if (req.body.street_address != null) {
          ...
          queryString +=
            " and patient.street_address=$" + (where.length + 1);
        }        

        if (req.body.first_name != null) {
          where.push(req.body.first_name);
          queryString +=
            " and patient.first_name=$" + (where.length + 1);
        }        

        if (req.body.middle_name != null) {
          where.push(req.body.middle_name);
          queryString +=
            " and patient.middle_name=$" + (where.length + 1);
        }        
        if (req.body.last_name != null) {
          where.push(req.body.last_name);
          queryString +=
            " and patient.last_name=$" + (where.length + 1);
        }        

        if (req.body.city != null) {
          where.push(req.body.city);
          queryString +=
            " and patient.city=$" + (where.length + 1);
        }        

        if (req.body.zip != null) {
          where.push(req.body.zip);
          queryString +=
            " and patient.zip=$" + (where.length + 1);
        }        

        if (req.body.state != null) {
          where.push(req.body.state);
          queryString +=
            " and patient.state=$" + (where.length + 1);
        }        
        ...
        try {
          ...
          return res.send({
            count : total_count ? total_count.rows?.length :0,
    ...
    app.post(
      ...
      async function (req, res) {
        ...
        console.log("1 ",where.length,'  ',where2.length);

        if (req.body.drug_code != null) {
          where2.push(req.body.drug_code);
          queryString2 +=
            " and prescription_drug.drug_code=$" + (where2.length );
        }



        console.log("2 ",where.length,'  ',where2.length);


        if (req.body.frequency != null) {
          where2.push(+req.body.frequency);
          queryString2 +=
            " and prescription_drug.frequency=$" + (where2.length );
        }


        console.log("3 ",where.length,'  ',where2.length);


        if (req.body.duration != null) {
          where2.push(+req.body.duration);
          queryString2 +=
            " and prescription_drug.duration=$" + (where.length );
        }



        console.log("4 ",where.length,'  ',where2.length);

        if (req.body.refills != null) {
          where2.push(+req.body.refills);
          queryString2 +=
            " and prescription_drug.refills=$" + (where2.length );
        }




        console.log("5 ",where.length,'  ',where2.length, queryString);


        if (req.body.hospital_id != null) {
          where.push(+req.body.hospital_id);
          queryString += " and prescription.hospital_id=$" + (where.length );
        }



        console.log("6 ",where.length,'  ',where2.length, queryString);

        if (req.body.hospital_patient_id != null) {
          where.push(+req.body.hospital_patient_id);
          queryString +=
            " and prescription.hospital_patient_id=$" + (where.length );
        }



        console.log("7 ",where.length,'  ',where2.length,  queryString);

        if (req.body.hospital_doctor_id != null) {
          where.push(+req.body.hospital_doctor_id);
          queryString +=
            " and prescription.hospital_doctor_id=$" + (where.length );
        }



        console.log("8 ",where.length,'  ',where2.length, queryString);        

        if (req.body.hospital_prescription_id != null) {
          where.push(+req.body.hospital_prescription_id);
          queryString +=
            " and prescription.hospital_prescription_id=$" + (where.length );
        }



        console.log("9 ",where.length,'  ',where2.length , queryString);

        if (req.body.hospital_pharmacy_id != null) {
          where.push(+req.body.hospital_pharmacy_id);
          queryString +=
            " and prescription.hospital_pharmacy_id=$" + (where.length);
        }

        console.log("10 ",where.length,'  ',where2.length , queryString);


        console.log(queryString,where);

        try {
          const prescriptions = await db
            .query(
              "SELECT prescription.id,prescription.hospital_id,prescription.hospital_prescription_id,prescription.hospital_patient_id,patient.first_name as patient_first_name,patient.middle_name as patient_middle_name,patient.last_name as patient_last_name,prescription.hospital_doctor_id,doctor.first_name as doctor_first_name, doctor.middle_name as doctor_middle_name,doctor.last_name as doctor_last_name, prescription.hospital_pharmacy_id,pharmacy.name as pharmacy_name from prescription,patient,doctor,pharmacy where prescription.hospital_id = patient.hospital_id and prescription.hospital_id = doctor.hospital_id and prescription.hospital_id = pharmacy.hospital_id and prescription.hospital_patient_id = patient.hospital_patient_id and prescription.hospital_doctor_id = doctor.hospital_doctor_id and prescription.hospital_pharmacy_id = pharmacy.hospital_pharmacy_id  " +
                queryString,
              [...where]
            )
            .catch((err) => {
          ...
          for (let i = 0; i < prescriptions.rows.length; i++) {
            let drugs = await db.findAll(
              ...
              {
                hospital_id: +prescriptions.rows[i].hospital_id,
      ...
      async function (req, res) {
        try {
          ...
          const prescription = await db
            .query(
              "SELECT prescription.id,prescription.hospital_id,prescription.hospital_prescription_id,prescription.hospital_patient_id,patient.first_name as patient_first_name,patient.middle_name as patient_middle_name,patient.last_name as patient_last_name,prescription.hospital_doctor_id,doctor.first_name as doctor_first_name, doctor.middle_name as doctor_middle_name,doctor.last_name as doctor_last_name, prescription.hospital_pharmacy_id,pharmacy.name as pharmacy_name from prescription,patient,doctor,pharmacy where prescription.hospital_id = patient.hospital_id and prescription.hospital_id = doctor.hospital_id and prescription.hospital_id = pharmacy.hospital_id and prescription.hospital_patient_id = patient.hospital_patient_id and prescription.hospital_doctor_id = doctor.hospital_doctor_id and prescription.hospital_pharmacy_id = pharmacy.hospital_pharmacy_id and prescription.id = $1;",
              [+req.body.id]
            )
            .catch((err) => {
              console.log(err);
          ...
          if (prescription && prescription.rows[0]) {
            let drugs = await db.findAll(
              ...
              {
                hospital_id: +prescription.rows[0].hospital_id,
                hospital_prescription_id:
                  +prescription.rows[0].hospital_prescription_id,
              }
            );
